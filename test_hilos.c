...
// Cerrojo para adquirir de forma exclusiva el derecho de acceso al Buffer.
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;

//Condición para gestionar la cola de espera para insertar/extraer del Buffer.
...
void *Productor(void *item){
	...
	//Si el Buffer está lleno no se puede producir hasta que un consumidor extraiga un item.
...
void *Consumidor(void *item){
	pthread_mutex_lock(&mutex);
	//Si no hay elementos que extraer espera a que un productor inserte un item.
...
int main(int argc, char *argv[]) {
  ...
  /*Para evitar los bucles infinitos de espera que se ocasionan cuando hay más productores que capacidad/consumidores
  o hay más consumidores que productores.*/
  if(np < nc) nc = np;
  else if(np > (nc + dim)) np = nc + dim;
