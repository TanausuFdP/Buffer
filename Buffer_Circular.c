...
void *Inserta_Item(void *elemento)
{
	
	Buffer->Buff[Buffer->Inserta] = (long)elemento;
	if ( (long)elemento % 2 )
		sleep(1);
	printf("Se ha insertado en %d el item %ld\n",Buffer->Inserta,(long)elemento);
...
void *Extrae_Item(void *elemento)
{	
	elemento = (void *)(Buffer->Buff[Buffer->Extrae]);
	Buffer->Buff[Buffer->Extrae] = -1;
	printf("Se extrajo de %d el item %ld\n",Buffer->Extrae,(long)elemento);
